...
<!-- Personal fork — studying the agent-plugin architecture for a side project. -->
- I'm primarily exploring the `vertical-plugins/` structure to understand how skills get bundled into agent plugins.
- Useful reminder: always run `sync-agent-skills.py` *after* `check.py` passes on the vertical source, not before.
